// Package encoder provides the dependency-light public JAB Code write path.
package encoder
...
// Encoder defaults shared by every public write entry point.
const (
...
// New returns an Encoder configured by opts.
...
func (e *Encoder) validate() error {
	if !validColorNumber(e.colors) {
		return fmt.Errorf("jabcode: invalid color number %d", e.colors)
	}
	if !e.format.Valid() {
		return fmt.Errorf("jabcode: invalid encoding format %d", e.format)
	}
	if e.format == wire.EncodeISO23634 && e.colors > 8 {
		return errors.New("jabcode: ISO/IEC 23634 reserves module color modes above 8 colors")
	}
