...
func TestISODefaultSymbolCorrection(t *testing.T) {
	r, err := encode.Render(encode.Config{
		Colors: 8, ModuleSize: 1, Profile: wire.ISO23634, SymbolNumber: 1,
	}, []byte("ISO/IEC 23634 conformance profile 0123456789"))
	if err != nil {
		t.Fatal(err)
	}
	...
	symbol := &core.DecodedSymbol{WireProfile: wire.ISO23634}
	...
	pg := (len(raw) / wr) * wr
	raw = raw[:pg]
	ecc.DeinterleaveProfile(raw, wire.ISO23634)
	if _, ok := ecc.DecodeLDPCHardProfile(raw, wc, wr, wire.ISO23634); !ok {
