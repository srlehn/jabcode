...
func DecodePrimaryMetadataPartI(matrix *core.Bitmap, symbol *core.DecodedSymbol, dataMap []byte, moduleCount, x, y *int) (ret int, syndromeOK bool) {
	...
	dec, syndromeOK := ecc.DecodeLDPCHardVariant(part1, wc, 0, symbol.WireVariant)
	if len(dec) < 3 {
...
func DecodePrimaryMetadataPartII(matrix *core.Bitmap, symbol *core.DecodedSymbol, dataMap []byte, normPalette, palThs []float64, moduleCount, x, y *int) (ret int, syndromeOK bool) {
	...
	dec, syndromeOK := ecc.DecodeLDPCHardVariant(part2, wc, 0, symbol.WireVariant)
	if len(dec) == 0 {
		return MetadataFailed, false
	}
...
func decodeSymbol(matrix *core.Bitmap, symbol *core.DecodedSymbol, dataMap []byte, normPalette, palThs []float64, typ int, trace *ModuleClassificationTrace) int {
	...
	ecc.DeinterleaveVariant(rawData, symbol.WireVariant)
	...
	dec, eccOK := ecc.DecodeLDPCHardVariant(rawData, wc, wr, symbol.WireVariant)
	if !eccOK || len(dec) != Pn {
