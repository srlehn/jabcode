...
func ReadColorPaletteInPrimary(matrix *core.Bitmap, symbol *core.DecodedSymbol, dataMap []byte, moduleCount, x, y *int) int {
	...
	if firstColor > 0 {
		for i := range copies {
			...
			writeColorPalette(matrix, symbol, i, tables.PrimaryPalettePlacementIndexVariant(i, 0, colorNumber, symbol.WireVariant)%colorNumber, p1.X, p1.Y)
			writeColorPalette(matrix, symbol, i, tables.PrimaryPalettePlacementIndexVariant(i, 1, colorNumber, symbol.WireVariant)%colorNumber, p2.X, p2.Y)
		}
	}
	...
	for colorCounter := firstColor; colorCounter < min(colorNumber, 64); colorCounter++ {
		for p := range copies {
			...
			writeColorPalette(matrix, symbol, p, tables.PrimaryPalettePlacementIndexVariant(p, colorCounter, colorNumber, symbol.WireVariant)%colorNumber, *x, *y)
