...
// flags word is consumed by the caller, not here; a candidate reaching this
// kernel has already been admitted.
...
// The pattern count, one count per finder type, then how many candidates found
// no slot. The host's fold has no bound of its own - its caller stops feeding
// it - so a drop here means the caller let more distinct patterns through than
// it should have, and silently keeping the first 500 would look like a
// successful fold.
