...
// streamPrior is a remembered winning hypothesis.
...
func (s *Stream) Decode(img image.Image) ([]byte, error) {
	...
	attempt := func(hyp streamHyp) ([]byte, bool) {
		...
		data, ok := s.attemptBitmap(bm)
		if ok {
			s.remember(streamPrior{side: key.side, deg: hyp.deg})
		}
		return data, ok
	}

	// Replay the most recent remembered hypothesis that has not been tried
	// this frame. A miss keeps the entry: geometry decays on repeated
	// disagreement, not on one bad frame.
	if len(s.ring) > 0 {
		s.work.replayAttempts++
		if data, ok := attempt(streamHyp{side: s.ring[0].side, deg: s.ring[0].deg}); ok {
			return data, nil
		}
	}
	...
	s.work.uprightScans++
	if data, ok := attempt(streamHyp{side: coarsestSide(levels, baseSide)}); ok {
		return data, nil
	}
	...
	if len(s.pending) == 0 {
		s.refillPending(img, levels, baseSide)
	}
	for len(s.pending) > 0 {
		hyp := s.pending[0]
		...
		break // budget: one selected attempt per frame
	}
...
// and interpret metadata, then - only when the admission gate passes and the
// frame's correction budget is unspent - one payload correction. There is no
// alignment-pattern fallback here: that second correction chain belongs to
// the single-image ladder. Docked secondaries decode only after a successful
// primary, as part of the committed read.
func (s *Stream) attemptBitmap(bm *core.Bitmap) (data []byte, ok bool) {
	...
	obs, stage := observePrimary(d, &symbols[0], nil)
	if stage != readSampled || obs == nil {
		return nil, false
	}
	if s.work.correctionChains >= 1 || !obs.AdmitPayloadCorrection() {
		return nil, false
	}
...
// remember moves a winning hypothesis to the ring's front, deduplicated and
// bounded.
func (s *Stream) remember(p streamPrior) {
	kept := s.ring[:0:0]
	kept = append(kept, p)
	for _, r := range s.ring {
		if r != p && len(kept) < streamRingCap {
			kept = append(kept, r)
		}
	}
	s.ring = kept
