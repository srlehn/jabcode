...
func TestDecodeCEncoded(t *testing.T) {
	...
	got, err := read.DecodeOnly(img, wire.CurrentC)
	if err != nil {
	...
	for _, colors := range []int{16, 32, 64, 128, 256} {
		for _, level := range []int{0, 3, 6} {
			for _, s := range payloads {
				img, err := NewEncoder(highColorOption(), WithColors(colors), WithECCLevel(level)).Encode([]byte(s))
				if err != nil {
				...
				if colors == 16 && level == 0 && s == payloads[0] {
					...
					if _, err := read.DecodeOnly(img, wire.ISO23634); err == nil {
...
func TestHighColorModePinned(t *testing.T) {
	...
	for _, colors := range []int{4, 8, 16, 32, 64, 128, 256} {
		img, err := NewEncoder(highColorOption(), WithColors(colors), WithModuleSize(1)).Encode([]byte("pin the mode"))
		if err != nil {
...
func TestEncodeColorLimits(t *testing.T) {
	...
	for _, colors := range []int{64, 128, 256} {
		multi := NewEncoder(highColorOption(), WithColors(colors), WithSymbols(
